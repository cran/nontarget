combine <-
function(pattern,adduct,homol=FALSE,rules=c(FALSE,FALSE,FALSE),dont=FALSE){
    ...
    if(length(pattern)>1 & length(adduct)>1){
      if((adduct[[2]][5]=="negative" & any(as.numeric(pattern[[11]])>0)) || (adduct[[2]][5]=="positive" & any(as.numeric(pattern[[11]])<0))){
      stop("Charges of adduct vs. pattern groups not consistent!")}}
    ############################################################################
    # (0.2) define local functions #############################################
          ...
          if(length(adduct[[1]])>1){ # if available ...
            ...
            if(length(thesepeaks)>0){
                ...
                if( length(addfrom)==1 & length(allpeaks)>1 ){ 
                  ...
                  # (3) adduct relative intensities ok? ########################
                  # ... skipped ################################################
                }
                ################################################################
    ...
    for(i in 1:length(comps[,1])){
      ...
      if(length(pattern)>1){
        ...
        intID<-c(intID,pattern[[1]][this,4][pattern[[1]][this,2]==max(pattern[[1]][this,2])[1]]);
        intmz<-c(intmz,pattern[[1]][this,1][pattern[[1]][this,2]==max(pattern[[1]][this,2])[1]]);
        intrt<-c(intrt,pattern[[1]][this,3][pattern[[1]][this,2]==max(pattern[[1]][this,2])[1]]);        
      }else{
        int<-c(int,max(adduct[[1]][this,2])[1]);
        intID<-c(intID,adduct[[1]][this,4][adduct[[1]][this,2]==max(adduct[[1]][this,2])[1]]);
        intmz<-c(intmz,adduct[[1]][this,1][adduct[[1]][this,2]==max(adduct[[1]][this,2])[1]]);
        intrt<-c(intrt,adduct[[1]][this,3][adduct[[1]][this,2]==max(adduct[[1]][this,2])[1]]);        
      }
      allin<-c(allin,this);
    }
